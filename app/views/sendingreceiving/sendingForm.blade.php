...
@if(Session::get('quick_sending_receiving')==1)
<div class='span12'>
	<strong>Click the button for one click sending. All item of godown will send to the stock.</strong>
	<br>
	<a href='{{ URL::to('quickSending')}}' class="btn btn-primary">Click Here for sending to Stock</a>
</div>
@else
<div class="span8">		
<div class='label label-info'>
	<h4><i class='icon-shopping-cart'></i> Sending to the Shop</h4>
</div>

			<div class="invoice-reg">
				{{ Form::open(array('route' => 'send.itemAddForsending', 'id' => 'formItemLocation', 'class' => 'form-horizontal')) }}
			...
			<table class="table table-striped" width="100%">
				...
				<tbody>
					...
					if(Session::get('sending')){
                               $reverse_items= array_reverse(Session::get('sending'));
                     } ?>
					@if(Session::get('sending'))
					   @foreach($reverse_items as $item)
						 <? $total_amount = $total_amount + $item['total_price']; ?>
						 {{ Form::open(array('route' => 'send.editDeleteSending', 'class' => 'form-horizontal')) }}
							<tr>
								<td>{{++$i}}</td>
								<td>{{$item['item_name']}}<input type="hidden" name="godown_item_id" value="{{$item['godown_item_id']}}"><input type="hidden" name="item_id" value="{{$item['item_id']}}"><input type="hidden" name="item_name" value="{{$item['item_name']}}"><input type="hidden" name="price_id" value="{{$item['price_id']}}"></td>
		...
		<div class="span4">
			<div class="invoice-right">
				<div>
					{{ Form::open(array('route' => 'send.saveSending', 'class' => 'form-horizontal')) }}
...
<script>
		 $().ready(function(){
			 ...
			 $("#auto_search_item").autocomplete("{{route('sending.itemAutoSuggest')}}", {
				width: 260,
				matchContains: true,
				queryDelay : 0,
