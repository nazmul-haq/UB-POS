...
@section('content')
	<div class="row">
	  <div class="span12">
		...
		{{ Form::open(array('route' => 'receive.saveReceiveItem', 'class' => 'form-horizontal', 'id' => 'form')) }}	
			...
			<tbody>				
				...
				@if($items)
					...
					@foreach($items as $item)
					 	<tr>
						   <td>
								<input class="check" type="checkbox" name="receiving_item_id[]" value="{{$item->receiving_item_id}}" >
						   </td>
						   
						   <td>{{$item->receiving_item_id}}</td>
						   <td>{{++$i}}</td>
						   <td>{{$item->item_id}}</td>
