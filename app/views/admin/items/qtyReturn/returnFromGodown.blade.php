...
if(Session::get('returnFromGodown')) {
    foreach(Session::get('returnFromGodown') as $item)
...
<div class="row">
    <div class="span8">
        ...
        <div class='label label-info'><h4><i class='icon-shopping-cart'></i> Quantity Return From Godown</h4></div>
        <div class="invoice-reg">
            <!-- {{Session::get('max_inv_dis_percent');}}!-->
				{{ Form::open(array('route' => 'admin.addReturnQtyFromGodown', 'id' => 'formItemLocation', 'class' => 'form-horizontal')) }}
...
<?php // $invoice_total=$invoice_total+$item['total'];?>
		  {{ Form::open(array('route' => 'admin.returnGodownEditDeleteItem', 'class' => 'form-horizontal')) }}
                <tr>
                    <td>{{++$i}}</td>
                    <td>
                        <span class="span3">
			{{$item['item_name']}}<input type="hidden" name="godown_item_id " value="{{$item['godown_item_id']}}">
        ...
        <div class="invoice-right">
                ...
                <div class="controls">
                    <table class="table table-striped" style="margin: 0; padding:0;">
                        <tbody>
			{{ Form::open(array('route' => 'admin.selectDeleteSupplierGodown', 'id' => 'supplierForm')) }}
					@if(Session::get('qty_godown_return_info.supp_or_comp_name'))
						<div class="control-group hr">											
							...
							<div class="controls">
								<strong style="color:#348100; font-size: 1.1em;">
									{{Session::get('qty_godown_return_info.supp_or_comp_name')}}
								</strong>
                                                                <input type="hidden" id="supplier" value="{{Session::get('qty_godown_return_info.qty_godown_return_info')}}">
								<p>
            ...
            <div>
		{{ Form::open(array('route' => 'admin.invoiceAndGodownReturn','autocomplete'=>'off', 'class' => 'form-horizontal')) }}
                <table class="table table-striped" style="margin: 0; padding:0;">
                    <tbody>
                        <tr>
                            <td>Date </td>
...
//Auto Complete for Item Search
         $("#auto_search_item").autocomplete("{{route('admin.godownItemAutoSuggestion')}}", {
                width: 260,
                matchContains: true,
                queryDelay : 0,
