    ...
    <div class="span8">
        ...
        <div class="invoice-reg"><input type="hidden" id="max_dis_percent" value="{{Session::get('max_inv_dis_percent');}}">
            <!-- {{Session::get('max_inv_dis_percent');}}!-->
				{{ Form::open(array('route' => 'saleReturn.returnItemAddTochart', 'id' => 'formItemLocation', 'class' => 'form-horizontal')) }}
            <div class="control-group">
						{{ Form::label('search_item', 'Find/Scan Invoice Id', ['class' => 'control-label', 'style' => 'font-weight: bold;']) }}
                <div class="controls">
							{{ Form::text('sale_invoice_id', null, array('class' => 'span6','autofocus'=>'yes','id'=>'auto_search_item', 'placeholder' => 'Start Typing or Scan Sale Invoice Id . . . . . . . . .')) }}
                </div> <!-- /controls -->
            </div> <!-- /control-group -->
				{{ Form::close() }}
        </div>
...
<?php // $invoice_total=$invoice_total+$item['total'];?>
							{{ Form::open(array('route' => 'saleReturn.editDeleteItem', 'class' => 'form-horizontal')) }}
                <tr>
                    <td>{{++$i}}</td>
                    <td>
                        <span class="span3">
										{{$item['item_name']}}<input type="hidden" name="i_sale_id" value="{{$item['i_sale_id']}}"><input type="hidden" name="item_id" value="{{$item['item_id']}}"><input type="hidden" name="price_id" value="{{$item['price_id']}}"><input type="hidden" name="item_name" value="{{$item['item_name']}}">
                        </span>
                    </td>
    ...
    <div class="span4">
        <div class="invoice-right">
            ...
            <div class="control-group hr">
                ...
                <div class="controls">
                    <table class="table table-striped" style="margin: 0; padding:0;">
                        <tbody>
						@if(Session::has('sale_return_invoice_info.customer_name'))
                            <tr>
                                <td width="112px">Customer Name</td>
                                <td>:</td>
                                <td><strong id="customer_name" style="color: green; font-size: 1.4em;">
					
                                        {{Session::get('sale_return_invoice_info.customer_name')}}
                            ...
                            <tr>
                                ...
                                <td>:</td>
                                <td style="padding:0;"><strong style="color: red; font-size: 1.2em;">&nbsp;&nbsp;{{Session::get('sale_return_invoice_info.customer_due')}}&nbsp;Tk</strong></td>
                            </tr>
                            <tr>
                                <td width="112px">Customer Point</td>
                                <td>:</td>
                                <td style="padding:0;"><strong style="color: red; font-size: 1.2em;">&nbsp;&nbsp;{{Session::get('sale_return_invoice_info.customer_point')}}&nbsp;</strong></td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
            ...
            <div>
		{{ Form::open(array('route' => 'saleReturn.invoiceAndSaleReturn','autocomplete'=>'off', 'class' => 'form-horizontal')) }}
                <table class="table table-striped" style="margin: 0; padding:0;">
                    <tbody>
                        @if(Session::get('backdate_return')==0)
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td><strong id="invoiced_discount"  font-size: 1.4em;">{{Session::get('sale_return_invoice_info.discount')}}</strong></td>
                        </tr>
                        @if(Session::get('sale_return_invoice_info.point_use_taka'))
                        <tr>
                            <td>Point Use Taka</td>
                            <td>:</td>
                            <td><strong id="point_use_taka"  font-size: 1.4em;">{{Session::get('sale_return_invoice_info.point_use_taka')}}</strong></td>
                        </tr>
                        @endif
                        <tr>
...
<script>
    $().ready(function(){
        ...
        $("#auto_search_item").autocomplete("{{route('saleReturn.invoiceAutoSuggest')}}", {
            width: 260,
            matchContains: true,
            queryDelay : 0
        });
