	  ...
	  <div class="span12">
		@include('_sessionMessage')
			<div class="invoice-reg print_disable">
                {{ Form::open(array('route' => 'deletePurchaseReportView', 'class' => 'form-horizontal')) }}
		...
		@if(isset($reports))
		{{ Form::open(array('route' => 'deletePurchaseInvoice.post', 'class' => 'form-horizontal','id' => 'submitForm')) }}
		<table class="table table-striped">
			...
			<button type="submit" class="pull-right btn btn-danger" id="deleteAll"><i class="icon-trash">Delete</i></button>
			...
			<tbody>
				...
				@if($reports)
				   ...
				   <?php $i++; ?>
					   <tr>
						   <td class="" id="check_{{$invoice->sup_invoice_id}}"><input type="checkbox" name="supInvoiceId_{{$i}}" value="{{$invoice->sup_invoice_id}}" />
						   <td>{{$i}}</td>
							<td>
